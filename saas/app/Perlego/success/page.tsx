...
export default function PerlegoSuccessPage() {
  ...
  const [usageEvents, setUsageEvents] = useState<number | ''>('');
    ...
    <main className="min-h-screen relative overflow-hidden" style={{ backgroundColor: 'white' }}>
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-8">
          ...
          <Link href="/Perlego" className="inline-flex items-center justify-center bg-blue-700 hover:bg-blue-800 text-white font-semibold py-2 px-5 rounded-xl shadow transition-colors">
            Manage Subscription
          </Link>
        </div>
              ...
              <section>
                ...
                <div className="rounded-xl p-6 border-2 border-gray-300" style={{ backgroundColor: 'white' }}>
                  <div className="flex justify-center">
                    <button className="action bg-blue-700 text-white hover:bg-blue-800" onClick={() => setUsageModal(true)}>Input Book Access</button>
                  </div>
                </div>
              </section>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900">Record Book Access</h2>
              <button
                onClick={() => !usageSubmitting && setUsageModal(false)}
                ...
                try {
                  ...
                  const resp = await fetch('/api/meter-operations', {
                    ...
                    body: JSON.stringify({
                      customerId,
                      date: usageDate,
                      value: Number(usageEvents)
                    })
                  });
                  const json = await resp.json();
                  if (!resp.ok || !json.success) {
                    throw new Error(json.error || 'Failed to register book access');
                  }
                  setUsageModal(false);
                  setUsageEvents('');
                } catch (err) {
            ...
            >
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Book Access Count</label>
                <input
                  type="number"
                  className="w-full px-3 py-2 border border-gray-300 rounded-md"
                  value={usageEvents}
                  onChange={(e) => setUsageEvents(e.target.value === '' ? '' : Number(e.target.value))}
                  min={0}
                  step={1}
                  required
                />
