...
export default function SuccessPayAsYouGoPage() {
  ...
  const [apiCallsModal, setApiCallsModal] = useState(false);
  const [apiCallsDate, setApiCallsDate] = useState(() => new Date().toISOString().slice(0,10));
  const [apiCallsValue, setApiCallsValue] = useState<number | ''>('');
  const [apiCallsSubmitting, setApiCallsSubmitting] = useState(false);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900">Record API calls</h2>
              <button
                onClick={() => !apiCallsSubmitting && setApiCallsModal(false)}
            ...
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                if (apiCallsValue === '' || !Number.isFinite(Number(apiCallsValue))) return;
                try {
                  setApiCallsSubmitting(true);
                  const resp = await fetch('/api/meter-events', {
                    ...
                    body: JSON.stringify({
                      ...
                      system: 'EventTest 19',
                      date: apiCallsDate,
                      value: Number(apiCallsValue)
                    })
                  });
                  const json = await resp.json();
                  if (!resp.ok || !json.success) {
                    throw new Error(json.error || 'Failed to register API calls');
                  }
                  setApiCallsModal(false);
                  setApiCallsValue('');
                } catch (err) {
                  console.error(err);
                } finally {
                  setApiCallsSubmitting(false);
                }
              }}
              className="space-y-4"
            >
              <div>
                ...
                <input
                  ...
                  value={apiCallsDate}
                  onChange={(e) => setApiCallsDate(e.target.value)}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">API Calls Value</label>
                <input
                  type="number"
                  className="w-full px-3 py-2 border border-gray-300 rounded-md"
                  value={apiCallsValue}
                  onChange={(e) => setApiCallsValue(e.target.value === '' ? '' : Number(e.target.value))}
                  min={0}
                  step={1}
                  required
                />
