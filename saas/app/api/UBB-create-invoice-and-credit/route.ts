...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Creating invoice and credit grant for UBB Core:', { customerId, invoiceAmount, creditAmount });
    ...
    const invoiceItemResponse = await fetch('https://api.stripe.com/v1/invoiceitems', {
      ...
      body: new URLSearchParams({
        ...
        'quantity': '1',
        'description': 'UBB Core Subscription Top Up',
        'invoice': invoice.id
      })
    });
    ...
    // Step 5: Create credit grant (monetary for UBB)
    console.log('Creating monetary credit grant for customer:', customerId, 'creditAmount:', creditAmount);

    // Create form-encoded body for Stripe API - using monetary amount for UBB
    const formBody = new URLSearchParams();
    formBody.append('amount[monetary][value]', (creditAmount * 100).toString()); // Convert dollars to cents
    formBody.append('amount[monetary][currency]', 'usd');
    formBody.append('amount[type]', 'monetary');
    ...
    return NextResponse.json({
      ...
      message: `Invoice for $${invoiceAmount} and credit grant of $${creditAmount} created successfully`
    });

  } catch (error) {
    console.error('Error in UBB-create-invoice-and-credit:', error);
    return NextResponse.json({
      success: false,
