...
export async function POST(request: NextRequest) {
  try {
    ...
    if (flowType === 'starter_invoice_custom_flow') {
    ...
    } else if (flowType === 'starter_custom_flow' || flowType === 'advanced_custom_flow' || flowType === 'pro_custom_credits_flow' || flowType === 'core_custom_credits_flow' || flowType === 'free_flow') {
      const flowName = flowType === 'starter_custom_flow' ? 'Starter' : flowType === 'free_flow' ? 'Free' : flowType === 'core_custom_credits_flow' ? 'Core' : 'Advanced';
      ...
      if (testClockId) {
        testClock = { id: testClockId };
        console.log('Using existing test clock:', testClockId);
      }

      // Customer metadata was already set during creation, just log
      console.log('Customer already configured with plan:', planType, 'and test clock:', testClockId);
    ...
    // Original billing flow for other plans
    // Now create the billing intent using the 2-step flow
        // Step 1 - Create billing cadence (different for Pro vs Starter)
    let cadenceOptions;
    
    if (flowType === 'advanced_custom_flow') {
      // Advanced flow: Monthly billing without specific day_of_month
      cadenceOptions = {
        payer: {
          type: "customer",
          customer: customer.id
      ...
      // Starter flow: Monthly billing on day 12
      cadenceOptions = {
        payer: {
          type: "customer",
          customer: customer.id
    ...
    if (flowType === 'free_flow') {
      // Free flow: Handle regular Stripe subscription
      console.log('Processing Free flow - regular Stripe subscription');
      console.log('Free flow - Session details:', {
        mode: session.mode,
      ...
      if ((session.mode as string) !== 'subscription' || !session.subscription) {
        throw new Error('Invalid free flow: expected subscription mode with subscription');
      }
      
      const subscription = typeof session.subscription === 'object' ? session.subscription : await stripe.subscriptions.retrieve(session.subscription);
      console.log('Free flow: Subscription created:', subscription.id);
      
      // Create invoice for price_1SD9Oy9geAvQAmidJE5r7PwH
      console.log('Free flow - Creating invoice for customer:', customer.id);
      ...
      if (!invoiceResponse.ok) {
        ...
        console.error('Invoice creation request details:', {
          customer: customer.id,
          auto_advance: 'true',
          collection_method: 'charge_automatically'
        });
      ...
      console.log('Free flow: Invoice created:', invoice.id);
      ...
      const invoiceItem = await invoiceItemResponse.json();
      console.log('Free flow: Invoice item created:', invoiceItem.id);
      ...
      console.log('Free flow: Invoice finalized:', finalizedInvoice.id);
      ...
      console.log('Free flow: Invoice paid:', paidInvoice.id);

      // Create credit grant for $100
      const creditGrantResponse = await fetch('https://api.stripe.com/v1/billing/credit_grants', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
      ...
      const creditGrant = await creditGrantResponse.json();
      console.log('Free flow: Credit grant created:', creditGrant.id);
      
      finalSubscription = subscription;
      invoiceId = paidInvoice.id;
      creditGrantId = creditGrant.id;
      
    } else if (flowType === 'advanced_custom_flow' || flowType === 'pro_custom_credits_flow' || flowType === 'core_custom_credits_flow') {
      // Advanced/Pro/Core flow: Get pricing plan details and create billing intent
      ...
      // Starter flow: Create rate card subscription
      
      // Step 2 - Create rate card subscription  
      // Determine the correct rate card ID based on flow type
      const actualRateCardId = 'rcd_test_61TBpoImAubZZiYxu16T5kls95SQJJF9DR1pbaQwq7WK';
      
      const flowName = 'Starter';
      console.log(`Creating ${flowName} rate card subscription with rate card:`, actualRateCardId, 'for plan type:', planType);
      
      const rateCardSubscriptionOptions = {
        rate_card: actualRateCardId,
        billing_cadence: cadence.id
      };

      console.log('Creating rate card subscription with options:', rateCardSubscriptionOptions);
      ...
      const rateCardSubscriptionResponse = await fetch('https://api.stripe.com/v2/billing/rate_card_subscriptions', {
        ...
        body: JSON.stringify(rateCardSubscriptionOptions)
      });

      if (!rateCardSubscriptionResponse.ok) {
        const errorText = await rateCardSubscriptionResponse.text();
        console.error('Rate card subscription creation failed:', errorText);
        throw new Error(`Rate card subscription creation failed: ${rateCardSubscriptionResponse.status} - ${errorText}`);
      }

      const rateCardSubscription = await rateCardSubscriptionResponse.json();
      console.log('Rate card subscription created:', rateCardSubscription);
      
      if (!rateCardSubscription || !rateCardSubscription.id) {
        throw new Error(`Rate card subscription creation failed: No ID returned. Response: ${JSON.stringify(rateCardSubscription)}`);
      }
      
      finalSubscription = rateCardSubscription;
    }

    // Prepare response based on flow type
    ...
    const isPricingPlanFlow = flowType === 'advanced_custom_flow' || flowType === 'pro_custom_credits_flow' || flowType === 'core_custom_credits_flow';
    const planName = isPricingPlanFlow ? 'Pricing Plan' : isFreeFlow ? 'Free Plan' : 'Rate Card Plan';
    
    return NextResponse.json({
      success: true,
      message: isFreeFlow 
        ? `Customer created with the customer id '${customer.id}' and subscription '${finalSubscription.id}'`
        : isCoreFlow
        ? `Customer ${customerName || customer.name} successfully subscribed to Core Plan!`
        : `Customer ${customerName || customer.name} successfully subscribed to ${planName}!`,
      customer: {
        id: customer.id,
        name: customerName || customer.name,
        email: customerEmail || customer.email,
      ...
      billing: isFreeFlow ? {
        ...
        priceId: 'price_1S0gTH9geAvQAmidisTN1O3w',
        invoiceId: invoiceId,
        creditGrantId: creditGrantId
      } : isCoreFlow ? {
        cadenceId: cadence.id,
        pricingPlanId: (session.metadata?.pricing_plan_id || 'bpp_test_61TMZYslMhAuWqiDV16T5kls95SQJJF9DR1pbaQwqC3M'),
        ...
        cadenceId: cadence.id,
        rateCardId: 'rcd_test_61TBpoImAubZZiYxu16T5kls95SQJJF9DR1pbaQwq7WK',
        rateCardSubscriptionId: finalSubscription.id,
        status: finalSubscription.status
      },
      // For the requested format: Customer ID and Subscription ID
      customerId: customer.id,
      subscriptionId: finalSubscription.id,
      testClock: testClock ? {
        id: testClock.id,
        ...
        frozenTime: testClock.frozen_time,
        status: testClock.status || 'active',
        note: (flowType === 'starter_custom_flow' || flowType === 'pro_custom_flow' || flowType === 'advanced_custom_flow' || flowType === 'starter_invoice_custom_flow') ? 
          "Test clock created at beginning of current day in London timezone (BST/GMT)" : 
          "Test clock created for reference - Billing v2 APIs don't directly support test clocks"
      } : undefined,
      session: {
        id: session.id,
