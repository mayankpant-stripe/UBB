...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting Core Custom Credits flow for:', { name, email });
    ...
    console.log('Core flow: Test clock created:', testClock.id);
    ...
    console.log('Core flow: Customer created:', customer.id);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        plan: 'core_plan',
        pricing_plan_id: 'bpp_test_61TMZYslMhAuWqiDV16T5kls95SQJJF9DR1pbaQwqC3M',
        test_clock_id: testClock.id,
        flow_type: 'core_custom_credits_flow'
      },
      custom_text: {
        submit: {
          message: 'Complete payment setup to activate your Core Plan subscription.'
        }
      }
    });

    console.log('Core flow: Checkout session created:', { id: session.id, customerId: customer.id, testClockId: testClock.id });
    ...
    console.error('Error in Core Custom Credits flow:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to create Core Custom Credits flow',
