...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting Pro Custom Credits flow for:', { name, email });
    ...
    console.log('Pro flow: Test clock created:', testClock.id);
    ...
    const customerResponse = await fetch('https://api.stripe.com/v1/customers', {
      ...
      body: new URLSearchParams({
        ...
        'metadata[created_via]': 'custom_credits_flow',
        'metadata[plan]': 'pro_plan',
        'metadata[pricing_plan_id]': 'bpp_test_61TDiJJYF0IjlFXST16T5kls95SQJJF9DR1pbaQwqE08',
    ...
    console.log('Pro flow: Customer created:', customer.id);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/newhack/customunitssuccess?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/newhack?checkout=cancelled`,
      metadata: {
        customer_name: name,
        customer_email: email,
        plan: 'pro_plan',
        pricing_plan_id: 'bpp_test_61TDiJJYF0IjlFXST16T5kls95SQJJF9DR1pbaQwqE08',
        test_clock_id: testClock.id,
        flow_type: 'pro_custom_credits_flow'
      },
      custom_text: {
        submit: {
          message: 'Complete payment setup to activate your Pro Plan subscription.'
        }
      }
    });

    console.log('Pro flow: Checkout session created:', { id: session.id, customerId: customer.id, testClockId: testClock.id });
    ...
    console.error('Error in Pro Custom Credits flow:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to create Pro Custom Credits flow',
