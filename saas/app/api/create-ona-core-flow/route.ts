...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting Ona Core flow for:', { name, email });
    ...
    console.log('Ona Core flow: Test clock created:', testClock.id);
    ...
    const customerResponse = await fetch('https://api.stripe.com/v1/customers', {
      ...
      body: new URLSearchParams({
        ...
        'metadata[created_via]': 'ona_core_flow',
        'metadata[plan]': 'ona_core',
        'metadata[pricing_plan_id]': 'bpp_test_61TOlflSUotHfMGCK16T5kls95SQJJF9DR1pbaQwq7pQ',
    ...
    console.log('Ona Core flow: Customer created:', customer.id);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/ona/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/ona?checkout=cancelled`,
      metadata: {
        customer_name: name,
        customer_email: email,
        plan: 'ona_core',
        pricing_plan_id: 'bpp_test_61TOlflSUotHfMGCK16T5kls95SQJJF9DR1pbaQwq7pQ',
        test_clock_id: testClock.id,
        flow_type: 'ona_core_flow'
      },
      custom_text: {
        submit: {
          message: 'Complete payment setup to activate your Ona Core Plan subscription.'
        }
      }
    });

    console.log('Ona Core flow: Checkout session created:', { id: session.id, customerId: customer.id, testClockId: testClock.id });
    ...
    console.error('Error in Ona Core flow:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to create Ona Core flow',
