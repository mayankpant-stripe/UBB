...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting Stability Pro Custom Credits flow for:', { name, email });
    ...
    console.log('Stability Pro flow: Test clock created:', testClock.id);
    ...
    console.log('Stability Pro flow: Customer created:', customer.id);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/Stability/customunitssuccess?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/Stability?checkout=cancelled`,
      metadata: {
        customer_name: name,
        customer_email: email,
        plan: 'stability_pro_plan',
        pricing_plan_id: 'bpp_test_61TRGAFBhWwiSLZp416T5kls95SQJJF9DR1pbaQwqQG0',
        test_clock_id: testClock.id,
        flow_type: 'stability_pro_custom_credits_flow'
      },
      custom_text: {
        submit: {
          message: 'Complete payment setup to activate your Stability Pro Plan subscription.'
        }
      }
    });

    console.log('Stability Pro flow: Checkout session created:', { id: session.id, customerId: customer.id, testClockId: testClock.id });
    ...
    console.error('Error in Stability Pro Custom Credits flow:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to create Stability Pro Custom Credits flow',
