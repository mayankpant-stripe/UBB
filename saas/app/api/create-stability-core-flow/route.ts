...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting Stability Core Custom Credits flow for:', { name, email });
    ...
    console.log('Stability Core flow: Test clock created:', testClock.id);
    ...
    console.log('Stability Core flow: Customer created:', customer.id);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/Stability/success_core?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/Stability?checkout=cancelled`,
      metadata: {
        customer_name: name,
        customer_email: email,
        plan: 'stability_core_plan',
        pricing_plan_id: 'bpp_test_61TR7HL9BnwPD8Aum16T5kls95SQJJF9DR1pbaQwq51c',
        test_clock_id: testClock.id,
        flow_type: 'stability_core_custom_credits_flow',
        invoice_amount: '10000000', // $100,000 in cents
        credit_units: '500000' // 500,000 custom units
      },
      custom_text: {
        submit: {
          message: 'Complete payment setup to activate your Stability Core Plan subscription.'
        }
      }
    });

    console.log('Stability Core flow: Checkout session created:', { id: session.id, customerId: customer.id, testClockId: testClock.id });
    ...
    console.error('Error in Stability Core Custom Credits flow:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to create Stability Core Custom Credits flow',
